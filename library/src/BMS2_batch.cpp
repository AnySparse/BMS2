...
int main(int argc, char** argv) {
  ...
  std::chrono::duration<double> total_host_wall_time{0}; // 总运行时间
  ...
  if (args.query_data) {
    ...
    // [新增 2] 捕获过滤后第一个查询图的节点数 (用于 --use-cs)
  ...
  while (!stop_processing) {
    ...
    // ==========================================
    // [Batch 内部逻辑 - 保持 sigmo2 结构]
    // ==========================================
    TimeEvents host_time_events; // 当前 Batch 的 Host 计时器
    host_time_events.add("setup_data_start");

    // 创建 Device Data Graph
    ...
    // Query 签名每次都需要重新生成 (因为 Signature 对象是新的)
    ...
    if (!args.skip_join) {
        ...
        if (partial_match_depth == 0) {
            ...
            // Partial + Expand Join (原 sigmo2 逻辑)
            ...
            // 对于分步 Join，sigmo2 逻辑中使用 Host 时间作为 GPU 时间的近似/替代
