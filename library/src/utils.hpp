...
public:
  ...
  Args(int& argc, char**& argv, sigmo::device::DeviceOptions& device_options) {
    ...
    options.add_options()
        ("p,print-candidates", "Print the number of candidates for each query node", cxxopts::value<bool>(print_candidates))
        ...
        ("skip-join", "Skip the join phase", cxxopts::value<bool>(skip_join))
        ("h,help", "Print usage")
        ...
        ("filter-work-group", "Set the work group size for the filter kernel. Default 512.", cxxopts::value<size_t>(device_options.filter_work_group_size))
        ("use-cs", "Set partial_match_depth to the node count of the first query graph", cxxopts::value<bool>(use_cs));
