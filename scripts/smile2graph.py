...
#把一个 RDKit 分子对象转为 networkx.Graph，节点是原子，边是化学键。
def molToGraph(mol):
  graph = nx.Graph()
...
#因为rdkit产生的结果是文字标签，所以要转换为离散值的图标签
#获取节点（原子）的标签：digit=True → 返回整数编号。digit=False → 返回原子符号。
...
#获取边（化学键）的标签编号。如果遇到新的键类型，则动态加入 bond_types。
...
#这个部分是用于将nx格式的图处理为每个模型需要的格式（真的吗？
class Parser:
...
'''
目的：把一批分子图导出为 SIGMO 框架需要的输入格式。

两种模式：
分组模式 (--group-diameter):按照图的直径（diameter）分组。每组补齐到相同数量（通过复制图）。每组单独输出到 query_d{diameter}.dat 文件。

普通模式:不分组，所有图顺序输出到同一个文件/标准输出。
